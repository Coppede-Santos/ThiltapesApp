...
public class MapActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    private void capturarSelecionado() {
        ...
        apiClient.capturar(
                ...
                ok -> {
            pendingCaptures.remove(thiltape.getId());
            btnCapturar.setEnabled(true);
            if (!ok) {
                Toast.makeText(this, R.string.captura_falhou, Toast.LENGTH_SHORT).show();
                return;
            }
            capturedInSession.add(thiltape.getId());
            btnCapturar.setVisibility(View.GONE);
            selectedThiltape = null;
            Toast.makeText(this, getString(R.string.captura_sucesso) + " " + thiltape.getNome(), Toast.LENGTH_SHORT).show();
            atualizarMapaEThiltapes();
            validarFimDeJogo();
                }
        );
